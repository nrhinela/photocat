...
export async function startIntegrationConnect(
  tenantId,
  provider,
  returnTo = '/app?tab=library&subTab=integrations',
  redirectOrigin = ''
) {
...
/**
 * Backward-compatible wrappers for Dropbox-specific integrations calls.
 */
export async function getDropboxIntegrationStatus(tenantId) {
  return getIntegrationStatus(tenantId);
}

export async function startDropboxIntegrationConnect(
  tenantId,
  returnTo = '/app?tab=library&subTab=integrations',
  redirectOrigin = ''
) {
  return startIntegrationConnect(tenantId, 'dropbox', returnTo, redirectOrigin);
}

export async function disconnectDropboxIntegration(tenantId) {
  return disconnectIntegration(tenantId, 'dropbox');
}

export async function updateDropboxIntegrationConfig(tenantId, payload = {}) {
  const normalized = {
    provider: payload.sourceProvider ?? payload.provider,
    syncFolders: payload.syncFolders,
    defaultSourceProvider: payload.defaultSourceProvider ?? payload.sourceProvider,
  };
  return updateIntegrationConfig(tenantId, normalized);
}
