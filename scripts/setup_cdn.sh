...
gcloud compute backend-buckets create photocat-thumbnails-backend \
...
gcloud compute url-maps create photocat-cdn-map \
    --default-backend-bucket=photocat-thumbnails-backend \
    --project=$PROJECT_ID || echo "URL map already exists"

# Create SSL certificate (managed)
echo "🔒 Creating SSL certificate..."
gcloud compute ssl-certificates create photocat-cdn-cert \
    --domains=cdn.photocat.app \
    --global \
    --project=$PROJECT_ID || echo "Certificate already exists (or use your own domain)"

# Create target HTTPS proxy
echo "🎯 Creating target proxy..."
gcloud compute target-https-proxies create photocat-cdn-proxy \
    --url-map=photocat-cdn-map \
    --ssl-certificates=photocat-cdn-cert \
    --project=$PROJECT_ID || echo "Proxy already exists"

# Reserve static IP
echo "📍 Reserving static IP..."
gcloud compute addresses create photocat-cdn-ip \
    --ip-version=IPV4 \
    --global \
    --project=$PROJECT_ID || echo "IP already exists"

# Get the IP address
CDN_IP=$(gcloud compute addresses describe photocat-cdn-ip --global --format="value(address)" --project=$PROJECT_ID)
echo "✅ Static IP: $CDN_IP"

# Create forwarding rule
echo "📡 Creating forwarding rule..."
gcloud compute forwarding-rules create photocat-cdn-rule \
    --address=photocat-cdn-ip \
    --global \
    --target-https-proxy=photocat-cdn-proxy \
    --ports=443 \
...
echo ""
echo "Next steps:"
echo "  1. Point your domain (cdn.photocat.app) to: $CDN_IP"
